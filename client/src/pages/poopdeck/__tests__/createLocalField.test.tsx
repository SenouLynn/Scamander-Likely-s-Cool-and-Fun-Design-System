...
describe("creates initial field", () => {
  ...
  it("adds initial pack to field", () => {
    const pack = buildPack({ pack: { label: "test" } });
  ...
  it("adds passed field to field", () => {
    ...
    expect(result[firstField].location).toBe(field[firstField].location);
  });
});
